...
if __name__ == '__main__':
    ...
    one_invasion_positive = (percent_ones>0).astype(float)
    ...
    preds_chesson1=one_invasion_positive[state_reached==4.0]
    true_values1=[1 for i in range(len(preds_chesson1))]
    preds_chesson1=np.array(preds_chesson1)
    true_values1=np.array(true_values1)
    true_positives1 = np.sum((true_values1 == 1) & (preds_chesson1 == 1))
    actual_positives1 = np.sum(true_values1 == 1)
    sensitivity1=true_positives1 / actual_positives1
    print('Sensitivity of Chesson criterion with SIV collapse region (at least one valid Chesson)')
    print(sensitivity1)
    ...
    one_invasion_positive[np.isnan(percent_ones)]=np.nan
    mean_invasion_growth=mean_list_arrays(data_replicates_invasion_growth)
    mask = (np.isclose(state_reached, 4.0)) & (~np.isnan(one_invasion_positive))
    preds_chesson2=one_invasion_positive[mask]
    preds_chesson2=np.array(preds_chesson2)
    true_values2=[1 for i in range(len(preds_chesson2))]
    true_values2=np.array(true_values2)
    true_positives2 = np.sum((true_values2 == 1) & (preds_chesson2 == 1))
    actual_positives2 = np.sum(true_values2 == 1)
    sensitivity2=true_positives2 / actual_positives2
    print('Sensitivity of Chesson criterion without SIV collapse region (at least one valid Chesson)')
    print(sensitivity2)
    ...
    axi=plot_with_scale(all_invasion_positive, cmap, mi, mx, atickx, aticky, alabel_tickx, alabel_ticky, 'Invasibility (valid Chesson across all replicates)')
