...
- example data in the folder `model_data/` from full or part of Figure 4, Figure 7a-o, Figure S6, Figure S14 and Figure S16 are stored in the folder `model_data` so these figures can be directly reproduced by running `python make_figures_scaled.py main_models` (generating the files `Scaled_PVZ_FFT_and_Period.pdf`,  `Scaled_PVZ_main_models.pdf`,  `Scaled_SIVZ_coexistence.pdf`, and  `Scaled_SIVZ_distances.pdf` in <10 seconds)  
