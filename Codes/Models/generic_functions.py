...
def concentration_ranges(indice, otype):
    ...
    if indice in [0,1] and otype in ['oligotrophic', 'mesotrophic']:
        ...
        if indice==0:
            I_cond_high=50
        I_cond_low=0.5
        if otype=='oligotrophic' and indice==1:
            I_cond_low=0
            I_cond_high=5
        perc_cond_high=50
        perc_cond_low=0
...
def phivs_encounter(virus_radius, Host_volume, Te,indice):
    ...
    if indice in [0,1]:
        ...
        phi_e_a=4*pi*(D_pred+D_prey)*(Host_radius*1e-6+virus_radius*1e-9)
        phi_e=4*pi*(D_pred+D_prey)*(Host_radius*1e-6+virus_radius*1e-9)
...
def round_sig(x, sig=2):
    if x == 0:
        return 0
    return round(x, sig - int(mt.floor(mt.log10(abs(x)))) - 1)
