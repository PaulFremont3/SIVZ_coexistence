...
if __name__ == '__main__':
    ...
    for k in to_loop:
        ...
        if type_r=='intracellular':
            vars_name=['phi','phi/phir','eps' ,'tot_res','dv', 'dv2', 'dz2', 'cost', 'ED_u', 'ED_m', 'ED_o', 'ED_av', 'ER_u', 'ER_m', 'ER_o', 'ER_av']
            vars_name_bis=['phi', 'eps' ,'phi/phir','dv', 'dv2', 'dz2', 'cost']
            if POM=='1':
                vars_name=['phi','phi/phir','eps' ,'tot_res','dv', 'dv2', 'dz2', 'cost', 'Pc','ED_u', 'ED_m', 'ED_o', 'ED_av', 'ER_u', 'ER_m', 'ER_o', 'ER_av']
        ...
        if type_r=='intracellular':
            if POM=='0':
                best_match=min_row.iloc[:, np.r_[0:7, 8:16,min_row.shape[1]-2 , min_row.shape[1]-1]]
            else:
                best_match=min_row.iloc[:, np.r_[0:8, 9:17,min_row.shape[1]-2,min_row.shape[1]-1]]
        else:
            if POM=='0':
                best_match=min_row.iloc[:, np.r_[0:6, 7:15, min_row.shape[1]-1]]
            else:
                best_match=min_row.iloc[:, np.r_[0:7, 8:18, min_row.shape[1]-1]]
        ...
        d3=df['ER_av']
        cor_mat=np.zeros((len(vars_name_bis)-1, len(vars_name_bis)-1))
        cor_mat[:]=np.nan
            ...
            for j, p2 in enumerate(vars_name_bis):
                if i>=j:
                    ...
                    if i!=0 and j!=0 and i>j:
                        if p_value<0.001:
                            if len(np.unique(d1))>3 and len(np.unique(d2))>3:
                                cor_mat[i-1, j-1]=r_value
        ...
        labels=vars_name_bis[1:]
        # Mask for upper triangle
        mask_upper = np.triu(np.ones_like(cor_mat, dtype=bool), k=1)
        
        print(cor_mat)
        # Mask for NaNs
        mask_nan = np.isnan(cor_mat)

        # Combine masks: upper triangle OR NaN
        mask = mask_upper | mask_nan

        # Red-to-blue diverging colormap, centered at 0
        ...
        for i in range(n):
            for j in range(n):
                if j >= i:
                    ax.add_patch(plt.Rectangle((j, i), 1, 1, color='white', ec='white'))
