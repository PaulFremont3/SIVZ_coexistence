...
if __name__ == '__main__':
    ...
    if dz2==0 and m2==0 and nyears==20:
        ...
        for k in to_plot:
            ...
            i1=0
            i2=round(365*5/dt)
            make_1_plot_SIVZ(S, I, V, Z, i1, i2, tit,dt,pp, Qp, Qv, Qz, 'no', ylab='Concentration (ind.L-1)', ymin=1, ymax=5e10)
            make_1_plot_SIVZ(S, I, V, Z, i1, i2, tit,dt,pp, 1, 1, 1, 'no', ylab='Concentration (umol.L-1)', ymin=1e-7, ymax=0.5)

            phi_rounded=round_sig(phi, 2)
            lp_rounded = round_sig(lp,2)
            write_vector(S, 'model_data/SIVZ_time_series_coex_S_'+str(phi)+'_'+str(lp_rounded)+'_'+suffix+'.txt',' ')
            write_vector(I, 'model_data/SIVZ_time_series_coex_I_'+str(phi)+'_'+str(lp_rounded)+'_'+suffix+'.txt',' ')
            write_vector(V, 'model_data/SIVZ_time_series_coex_V_'+str(phi)+'_'+str(lp_rounded)+'_'+suffix+'.txt',' ')
            write_vector(Z, 'model_data/SIVZ_time_series_coex_Z_'+str(phi)+'_'+str(lp_rounded)+'_'+suffix+'.txt',' ')
    ...
    for phi in phi_to_plot:
        ...
        write_vector(S, 'model_data/SIVZ_time_series_S_'+str(phi)+'_'+suffix+'.txt',' ')
        write_vector(I, 'model_data/SIVZ_time_series_I_'+str(phi)+'_'+suffix+'.txt',' ')
        write_vector(V, 'model_data/SIVZ_time_series_V_'+str(phi)+'_'+suffix+'.txt',' ')
        write_vector(Z, 'model_data/SIVZ_time_series_Z_'+str(phi)+'_'+suffix+'.txt',' ')
    pp.close()
    ...
    if not np.isnan(ind_min):
        ...
        if not np.isnan(ind_min_val):
            ...
            if phi_a_index_par==phi_index_par:
                valid_concentrations_5[phi_index_par, lp_index_par]=4
            else:
                valid_concentrations_5[phi_index_par, lp_index_par]=4
                valid_concentrations_5[phi_a_index_par, lp_index_par]=5
            cond=(np.nan_to_num(distance_to_target_val, nan=np.inf) > min_d_val + 10) & (np.nan_to_num(valid_concentrations_5, nan=np.inf) != 3) & (np.nan_to_num(distance_to_target_val, nan=np.inf) < min_d_val + 75)
            cmap = matplotlib.colors.ListedColormap(['crimson', 'royalblue', 'darkorange','gold', 'limegreen', 'cyan'])
            valid_concentrations_5[valid_concentrations_5==2]=3
            valid_concentrations_5[cond]=2
            mx=5
            print('here')
            print(valid_concentrations_5)
            norm=mcolors.BoundaryNorm(boundaries=[-0.5, 0.5, 1.5, 2.5, 3.5, 4.5, 5.5], ncolors=6)
            plot_with_scale(valid_concentrations_5, cmap, mi, mx, atickx, aticky, alabel_tickx, alabel_ticky, 'Valid concentrations + % infected + % virus kill + V/A ratio',norm=norm, yl=ylb)
            pp.savefig()
    ...
    if nyears==20:
        ...
        write_matrix(final_Surv, 'model_data/state_reached_'+type_mod+'_'+suffix+'.txt', ' ')
        write_matrix(valid_concentrations_5, 'model_data/valid_concentrations_and_all_constraints_'+type_mod+'_'+suffix+'.txt', ' ')

        nas=['valid_concentrations', 'valid_concentrations_and_pinf',  'valid_concentrations_and_pkill', 'valid_concentrations_and_vp_ratio']
        co=0
        for mat in to_plot[0:4]:
            na=nas[co]
            write_matrix(mat, na+'_'+type_mod+'_'+suffix+'.txt', ' ')
            co=co+1

    if 'predicted_state_bis' in globals():
        write_matrix(final_pers, 'model_data/Period_'+type_mod+'_'+suffix+'.txt', ' ')
        write_matrix(final_mods, 'model_data/FFT_modulo_'+type_mod+'_'+suffix+'.txt', ' ')
        ...
        write_matrix(predicted_state_bis, 'model_data/predicted_state_th_'+type_mod+'_'+suffix+'.txt', ' ')
        write_matrix(alter_state, 'model_data/alternate_stable_state_th_'+type_mod+'_'+suffix+'.txt', ' ')
        write_matrix(u_state, 'model_data/unstable_equilibria_th_'+type_mod+'_'+suffix+'.txt', ' ')
        write_matrix(limit_val_mat, 'model_data/limit_val_th_'+type_mod+'_'+suffix+'.txt', ' ')
