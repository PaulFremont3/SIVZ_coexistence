...
if __name__ == '__main__':
    ...
    replicate=int(sys.argv[7]) # replicate no
    ...
    suffix=typePhyto+'_BS'+str(bs)+'_LOI'+eff+'_replicate_'+str(replicate)
    if dz2==0:
        suffix+='_no-dz2'
    if dv2==0:
        suffix+='_no-dv2'
    ...
    titles_0=['invasion_growth_rate','fluctuation_free_growth_rate', 'relative_non_linearity_in_S', 'relative_non_linearity_in_I', 'IS_covariance', 'IS_variance_interaction']
    for l in range(ntot):
    ...
    mx=np.nanmax(np.array(invasibilty_both))
    mi=np.nanmin(np.array(invasibilty_both))
    ...
    mi=-amx
    ma=amx
    plot_with_scale(invasibilty_both, cmap0,mi,ma, atickx, aticky, alabel_tickx, alabel_ticky, 'Total invasion growth rate', norm=norm)
    pp.savefig()
    ...
    invasibilty_both_supp0=copy.deepcopy(invasibilty_both)
    invasibilty_both_supp0[invasibilty_both_binary==0]=np.nan
    bounds=np.linspace(0, amx, 100)
    norm = matplotlib.colors.BoundaryNorm(bounds, cmap0.N-1)
    mi=0
    ma=amx
    plot_with_scale(invasibilty_both_supp0, cmap0,mi,ma, atickx, aticky, alabel_tickx, alabel_ticky, 'Total invasion growth rate (>0)', norm=norm)
    pp.savefig()
    ...
    for l in range(ntot):
        ...
        mat[mat<-100]=-100
        mat[mat>100]=100
        write_matrix(mat, 'model_data/SIVZ_MCT_'+suffix+'_'+titles_0[l]+'_stab.txt', ' ')
        mat=matrices_effects_z_inv[l]
        mat[mat<-100]=-100
        mat[mat>100]=100
        write_matrix(mat, 'model_data/SIVZ_MCT_'+suffix+'_'+titles_0[l]+'_Z_inv.txt', ' ')
        mat=matrices_effects_v_inv[l]
        mat[mat<-100]=-100
        mat[mat>100]=100
        write_matrix(mat, 'model_data/SIVZ_MCT_'+suffix+'_'+titles_0[l]+'_V_inv.txt', ' ')

    write_matrix(invasibilty_both_binary, 'model_data/SIVZ_MCT_'+suffix+'_chesson_binary.txt', ' ')
    write_matrix(invasibilty_both, 'model_data/SIVZ_MCT_'+suffix+'_chesson_growth.txt', ' ')
