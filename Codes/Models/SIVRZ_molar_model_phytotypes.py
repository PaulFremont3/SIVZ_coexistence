...
if __name__ == '__main__':
  ...
  if param=='phir' or param=='epsr':
    ...
    for phi in phis_to_plot:
      ...
      write_vector(S, 'model_data/SIVRZ_time_series_partial-res_S_'+str(phi)+'_'+suffix+'.txt',' ')
      write_vector(I, 'model_data/SIVRZ_time_series_partial-res_I_'+str(phi)+'_'+suffix+'.txt',' ')
      write_vector(V, 'model_data/SIVRZ_time_series_partial-res_V_'+str(phi)+'_'+suffix+'.txt',' ')
      write_vector(R, 'model_data/SIVRZ_time_series_partial-res_R_'+str(phi)+'_'+suffix+'.txt',' ')
      write_vector(Z, 'model_data/SIVRZ_time_series_partial-res_Z_'+str(phi)+'_'+suffix+'.txt',' ')
    ...
    for phi in phis_to_plot:
      ...
      i3=round((365)/dt)
      make_plots_SIVRZ(S, I, V, R,Z, i1, i3, tit,dt,pp, Qp, Qv, Qz)
      write_vector(S, 'model_data/SIVRZ_time_series_full-res_S_'+str(phi)+'_'+suffix+'.txt',' ')
      write_vector(I, 'model_data/SIVRZ_time_series_full-res_I_'+str(phi)+'_'+suffix+'.txt',' ')
      write_vector(V, 'model_data/SIVRZ_time_series_full-res_V_'+str(phi)+'_'+suffix+'.txt',' ')
      write_vector(R, 'model_data/SIVRZ_time_series_full-res_R_'+str(phi)+'_'+suffix+'.txt',' ')
      write_vector(Z, 'model_data/SIVRZ_time_series_full-res_Z_'+str(phi)+'_'+suffix+'.txt',' ')
  ...
  if not np.isnan(ind_min):
    if param in ['lp_epsr', 'lp_phir']:
      ...
      if not np.isnan(ind_min_val):
        ...
        plot_with_scale(valid_concentrations_5, cmap, mi, mx, atickx, aticky, alabel_tickx, alabel_ticky, 'Valid concentrations + % infected + % virus kill + V/A ratio', norm=norm)
        pp.savefig()
  ...
  if nyears==20:
    ...
    write_matrix(final_state_all, 'model_data/state_reached_'+type_m+'_'+suffix+'.txt', ' ')
    write_matrix(valid_concentrations_5, 'model_data/valid_concentrations_and_all_constraints_'+type_m+'_'+suffix+'.txt', ' ')
    ...
    for mat in to_plot[0:4]:
      na=nas[co]
      write_matrix(mat, na+'_'+type_m+'_'+suffix+'.txt', ' ')
      co=co+1  
  
  if 'predicted_state' in globals():
    ...
    write_matrix(predicted_state, 'model_data/predicted_state_th_'+type_m+'_'+suffix+'.txt', ' ')
    write_matrix(alter_state, 'model_data/alternate_stable_state_th_'+type_m+'_'+suffix+'.txt', ' ')
    write_matrix(u_state, 'model_data/unstable_equilibria_th_'+type_m+'_'+suffix+'.txt', ' ')
    write_matrix(limit_val_mat, 'model_data/limit_val_th_'+type_m+'_'+suffix+'.txt', ' ')
