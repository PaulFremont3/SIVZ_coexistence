...
for (i in 0:3){
  ind_mx_mu=which.max(mu_max[(i*100+1):(i*100+100)])
  if (i!=0){
    bs=pred_BS[(i*100+1):(i*100+100)][ind_mx_mu]
  } else{
    bs=pred_BS_gam[(i*100+1):(i*100+100)][ind_mx_mu]
  }
  bs_lm=pred_BS_lm[(i*100+1):(i*100+100)][ind_mx_mu]
  
  lp=pred_LP[(i*100+1):(i*100+100)][ind_mx_mu]
  lp_lm=pred_LP_lm[(i*100+1):(i*100+100)][ind_mx_mu]
  
  Qp=Qps[(i*100+1):(i*100+100)][ind_mx_mu]
  
  print(types[i+1])
  Hv=Host_volumes[(i*100+1):(i*100+100)][ind_mx_mu]
  print('host volume:')
  print(Hv)
  print('host radius')
  Hr=(3*Hv/(4*pi))^(1/3)
  ...
  print('mu_ward')
  print(mu_max_bis[(i*100+1):(i*100+100)][ind_mx_mu])
  
  print('mu_dut')
  print(mu_max_ter[(i*100+1):(i*100+100)][ind_mx_mu])
  print(' ')
}
...
plot(Host_volumes, final_preds_LP/24, log='x',col=new_data_gam$col, pch=19,  axes=FALSE,frame.plot=TRUE , xlab='V', ylab='Model latent period')
axis(1, at=log10Tck('x','major'), tcl= 0.2) # bottom
axis(1, at=log10Tck('x','minor'), tcl= 0.1, labels=NA)
axis(2)
