best_models_gam<-function(id){
  ...
  optimisation_gam <- function(i, gamGrid, variables, nsp, df_full){
    ...
    possibleError <- tryCatch(
      gam_model <- mgcv::gam(y ~ s(log10Vhost, k=nsp) + s(RVirus, k=nsp)+VirusType+HostType ,
                             data = df_full, family = 'gaussian',mehtod='ML', optimizer = c('outer', 'newton'),
                             control = mgcv::gam.control(epsilon = 10^-6)),
    ...
    cv_gam <- function(sample,i, nsp, df_full){
      ...
      set.seed(42)
      gam_model_cv <- mgcv::gam(y ~ s(log10Vhost, k=nsp)   + s(RVirus, k=nsp)+VirusType+HostType ,
                                data = df3)
